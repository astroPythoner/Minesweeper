...
namespace Minesweeper
{
    ...
    public class Steuerung
    {
        ...
        private void end_game(bool won)
        {
            ...
            string time_string = String.Format("{0:00}:{1:00}min", sw.Elapsed.Minutes, sw.Elapsed.Seconds);
            if (won) {
                // Gewonnen MessageBox
                if (num_hints == 0)
                {
                    gui.show_message_box("Sehr gut gemacht! Du hast es ohne Hinweise geschafft! Zeit: " + time_string + "\n ", "Spiel gewonnen");
                }
                else
                {
                    gui.show_message_box("Gut gemacht! Du hast es mit "+num_hints+" Hinweisen geschafft! Zeit: " + time_string + "\n ", "Spiel gewonnen");
                }
            }
            else
            {
                // Verloren MessageBox
                gui.show_message_box("Das war wohl nichts! Zeit: " + time_string, "Spiel verloren");
            }
